...
async function main() {
  ...
  try {
    // Try to create cars directly - this will fail if tables don't exist
    const createdCars = [];
    for (const car of cars) {
      try {
        ...
        if (error.code === 'P2021') {
          ...
          console.log(`⚠️ Error creating ${car.make} ${car.model}:`, error.message);
        }
      }
    }

    if (createdCars.length > 0) {
      console.log(`✅ Successfully created ${createdCars.length} cars`);
    }
  } catch (error) {
